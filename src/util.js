...
export async function perfStats() {
    ...
    return { cpu, mem, net_rx, net_tx, net_sp };
}
...
export function readableBytes(bytes = 0, units = ['B', 'KB', 'MB', 'GB', 'TB']) {
    let i = 0;
...
class LoadAvgSimulator {
    constructor() {
        this.windows = {
            '1m': { duration: 60, samples: [] },   // 1分钟窗口（假设每秒采样一次，存60个点）
            '5m': { duration: 300, samples: [] },  // 5分钟窗口
            '15m': { duration: 900, samples: [] }, // 15分钟窗口
        };
        this.previousCpuTime = { total: 0, idle: 0 };
        this.startSampling();
    }
    ...
    getLoadAvg() {
        ...
        return [
            averages['1m'] * os.cpus().length,  // 乘以 CPU 核心数以模拟系统负载
            averages['5m'] * os.cpus().length,
            averages['15m'] * os.cpus().length,
        ];
    }
